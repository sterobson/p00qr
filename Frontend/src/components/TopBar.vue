<template>
  <div class="top-row">
    ...
    <span id="code-label" v-if="codeLabel">{{ codeLabel }}</span>
    <button v-if="!codeLabel" @click="$emit('pick-token')" id="pick-token-btn">Pick a token</button>
    <button
      v-if="historyCount > 0 && !codeLabel"
...
defineEmits(['menu-toggle', 'history-open', 'history-close', 'pick-token'])
